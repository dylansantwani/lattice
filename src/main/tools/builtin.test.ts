...
describe('run_agent (subagent delegation)', () => {
  ...
  it('rejects an empty task', async () => {
    const withSpawner = { ...ctx, runSubagent: async () => ({ text: '', agentId: 'a', toolCalls: 0 }) }
  ...
  it('delegates to the spawner and returns its result to the caller', async () => {
    ...
    const withSpawner = {
      ...
      runSubagent: async (spec: { task: string; agentType?: string; model?: string }) => {
        calls.push(spec)
        return { text: 'the answer is 42', agentId: 'agent_1', toolCalls: 3 }
      }
    }
    ...
    expect(calls).toEqual([
      { task: 'find the answer', agentType: 'researcher', model: 'cc/claude-opus-5', effort: undefined }
    ])
    expect(res).toEqual({ agentId: 'agent_1', toolCalls: 3, result: 'the answer is 42' })
  })
})
