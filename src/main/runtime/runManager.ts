...
async function executeRun(
  ...
  try {
    ...
    while (continueLoop) {
      ...
      if (run.steerQueue.length > 0 && !run.abort.signal.aborted) {
        ...
        wire.splice(0, wire.length, ...buildWireMessages(threadId, meta))
        continueLoop = true
        flush()
      }
    }
  ...
  if (meta.title === 'New thread' && !errored) {
    ...
    if (firstUser) {
      ...
      const summary = await generateTitle(model, firstUser.text, text)
...
async function generateTitle(model: string, userText: string, assistantText: string): Promise<string | null> {
  const provider = resolveProvider()
...
async function runSubagentLoop(
  ...
  emit({ type: 'run.started', model, effort, mode: meta.mode, parentAgent: parent.runId })
  ...
  // strip both from their tool set.
  const tools = availableTools(meta).filter((t) => t.name !== 'run_agent' && t.name !== 'ask_user')
  ...
  return { text, agentId, toolCalls, telemetry }
}
