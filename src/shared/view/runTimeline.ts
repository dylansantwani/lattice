import type { ReasoningFidelity, RunEvent, RunEventBody } from '../types'
...
export type TimelineItem =
  | {
      ...
      durationMs?: number
      /** Reasoning tokens the provider billed for this bout. Set only for a bout that arrived as a
       *  count instead of as text (`silent`), where it is the only measure of how much thinking
       *  happened — a bout with readable text is measured by the text itself. */
      tokenCount?: number
      /** This bout was never readable: the provider reported thinking only in its usage block, so
       *  there is no reasoning text and never will be. The transcript says so rather than offering
       *  an expander onto nothing. */
      silent?: boolean
    }
...
export function buildTimeline(events: RunEvent[]): TimelineItem[] {
  ...
  /** The event ts each item was pushed at, parallel to `items` — the ordering key a spliced-in
   *  silent bout is placed against (an item's own `startTs` can predate its event). */
  const itemTs: number[] = []
  ...
  for (const ev of sorted) {
    ...
    if (b.type === 'reasoning.delta') {
      ...
      if (think) {
        ...
        // A bout that was never readable (see the `reasoning.done` docs in shared/types): the
        // provider reported thinking as a token count only, so no delta ever opened a segment. The
        // run loop emits this at ROUND END, but the thinking happened at the round's START — before
        // the output and tool rows already sitting in `items` — so it is spliced back to where it
        // belongs rather than appended after the work it preceded.
        const bout: TimelineItem = {
          kind: 'think',
          seq: ev.seq,
          text: '',
          startTs: b.startedAt,
        ...
        let at = itemTs.findIndex((ts) => ts > b.startedAt!)
        if (at < 0) at = items.length
        items.splice(at, 0, bout)
        itemTs.splice(at, 0, b.startedAt)
        // Everything indexed by position has shifted right of the insertion point.
        for (const [id, idx] of toolIndex) if (idx >= at) toolIndex.set(id, idx + 1)
        if (commitIndex >= at) commitIndex++
      }
