...
export function Composer(): React.JSX.Element {
  ...
  const onKeyDown = (e: React.KeyboardEvent): void => {
    ...
    if (e.key === 'Enter') {
      // Which chord sends depends on the sendKey preference:
      //  - 'enter'     → Enter sends; ⌘/Ctrl+Enter queues; Shift+Enter is a newline.
      //  - 'mod-enter' → ⌘/Ctrl+Enter sends; a bare Enter inserts a newline.
      const mod = e.metaKey || e.ctrlKey
      const wantsSend = sendKey === 'mod-enter' ? mod : !e.shiftKey && !mod
      const wantsQueue = sendKey === 'enter' && mod && !e.shiftKey
      if (!wantsSend && !wantsQueue) return // Shift+Enter, or a bare Enter in mod-enter mode → newline
