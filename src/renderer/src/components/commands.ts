...
/**
 * The command-name token of a slash input: `/` followed by non-space characters with
 * nothing after it yet (this is the state in which the palette is open). Returns the
 * token (without the slash), or null when the text is not a bare command token.
 */
...
/** Split a fully-typed `/name arg…` line into its resolved command and (left-trimmed) argument. */
