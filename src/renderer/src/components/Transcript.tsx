...
/**
 * The compaction summary that stands in for earlier, now-dimmed history. Collapsed by
 * default (the divider tells the reader compaction happened); expands to the full summary.
 */
    ...
    <div className="compaction">
      ...
      >
        ...
        <span className="compaction-label">
          <I name="compress" size={13} /> Context compacted
        </span>
        <I name={open ? 'expand_less' : 'expand_more'} size={15} className="compaction-chev" />
        <span className="compaction-rule" />
      </div>
      {open && (
...
function ThinkingSegment({
  ...
  return (
    <div className={`thinking-card ${live ? 'live' : 'done'}`}>
      ...
      {open && hasText && <div className="thinking-log">{text}</div>}
    </div>
  )
}
...
function ToolRow({ call }: { call: ToolCall }): React.JSX.Element {
  ...
  return (
    <div className={`tool-activity-row ${call.status} ${open ? 'open' : ''}`}>
      ...
      {/* A file mutation shows its +/− diff inline, like a desktop diff viewer. */}
      {diff && <FileDiff path={diff.path} before={diff.before} after={diff.after} kind={diff.kind} />}
      {open && (
