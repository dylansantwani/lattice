...
export function FileDiff({
  ...
  after: string
  kind: 'edit' | 'write'
  /** Fired when the user clicks the diff header — lets the shell react (e.g. open the inspector). */
  onActivate?: () => void
}): React.JSX.Element {
  ...
  return (
    <div className={`filediff ${open ? 'open' : ''}`}>
      <button
        className="filediff-head"
        onClick={() => {
          setOpen((v) => !v)
          onActivate?.()
        }}
        aria-expanded={open}
      >
        <span className="filediff-verb">{kind === 'write' ? 'Created' : 'Edited'}</span>
        ...
        <span className="msym filediff-chev" style={{ fontSize: 16 }}>
