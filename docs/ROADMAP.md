...
- [x] Built-in tools: fs_read, fs_write, fs_edit, fs_list, shell, grep_search, todo_write, memory_save, memory_search (`src/main/tools/builtin.ts`)
- [ ] Permission broker: mode ceiling (Plan strips mutating tools) → saved rules → preset defaults (manual/workspace/full); precedence per plan §"Mode and permission controls"
- [ ] Tool loop in runManager: accumulate tool_call deltas → propose → broker decide → (await approval) → execute → tool.result event → continue loop; in-run wire history kept in memory
...
- [ ] Spine UI for tool events (compact cards: args summary, result, duration); Tasks inspector showing run checklist from todo_write
